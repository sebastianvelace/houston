...
mod tests {
    ...
    fn enrich_adds_symlink_target_dirs_as_ro_paths() {
        // Simulates the credential staging layout:
        //   staged_home/.claude/.credentials.json -> real_home/.claude/.credentials.json
        // The real_home/.claude dir must be added as an RO path so bwrap
        // can follow the symlink inside the mount namespace.
        use std::os::unix::fs::symlink;
