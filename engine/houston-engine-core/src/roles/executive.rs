//! Executive manager: aggregate connected agents, then brief the owner.
...
const DEFAULT_EXECUTIVE_CLAUDE_MD: &str = "## Instructions\n\n\
You are the executive assistant for the business owner. \
You aggregate updates from team agents and present a clear, actionable briefing.\n\n\
## Learnings\n";
...
impl From<ExecutiveError> for CoreError {
    fn from(err: ExecutiveError) -> Self {
        match err {
            ...
            ExecutiveError::AgentNotFound(name) => {
...
/// Build the sync-session prompt sent to each connected agent.
...
/// Build the main executive session prompt from connected-agent responses.
...
/// Ensure the configured executive agent folder exists, seeding it when missing.
...
/// Seed the executive agent for every workspace under `workspaces_root`.
pub fn ensure_executive_agents_for_all_workspaces(
    root: &Path,
    workspaces_root: &Path,
) -> Result<(), CoreError> {
    let workspaces = crate::workspaces::read_all(workspaces_root)?;
...
/// Validate that every connected agent name exists in the workspace.
...
/// Ensure the executive agent folder from `config` exists, creating it when missing.
...
/// Persist executive config after validating connected agents and ensuring the executive agent exists.
