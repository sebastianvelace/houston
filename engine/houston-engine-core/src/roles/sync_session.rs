//! Synchronous (non-streaming) provider sessions for orchestration sub-requests.

use crate::sessions::resolve_provider;
...
/// Run a one-shot session against `agent_dir` and return the final assistant text.
...
/// Build the provider prompt for a data-provision sub-session.
pub fn build_provision_prompt(provides_id: &str, description: &str) -> String {
    format!(
        "Proporciona el {provides_id}: {description}. Responde solo con los datos, sin contexto adicional."
    )
}
...
mod tests {
    ...
    #[test]
    fn provision_prompt_includes_id_and_description() {
        let prompt = build_provision_prompt("financial_summary", "Revenue and expenses");
        assert!(prompt.contains("financial_summary"));
        assert!(prompt.contains("Revenue and expenses"));
    }
}
