...
import { ChatPanel, mergeFeedHistory } from "@houston-ai/chat";
import {
  Empty,
  EmptyDescription,
  EmptyHeader,
...
export function ExecutiveManagerView() {
  ...
  useEffect(() => {
    ...
    tauriChat
      .loadHistory(directorPath, sessionKey)
      .then((history) => {
        if (history.length === 0) return;
        const current =
          useFeedStore.getState().items[directorPath]?.[sessionKey] ?? [];
        setFeed(
          directorPath,
          sessionKey,
          mergeFeedHistory(history as FeedItem[], current),
        );
      })
      .catch(() => {});
    // Run only when the key/path combo changes, not on every render.
  ...
  const handleSend = useCallback(
    async (text: string) => {
      ...
      pushFeedItem(path, key, { feed_type: "user_message", data: text } as FeedItem);
      ...
      <header className="flex shrink-0 items-center gap-3 border-b border-black/5 px-6 py-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {director ? director.name : t("executive:chat.directorMissingTitle")}
          </p>
        </div>
      </header>
                ...
                <div className="mx-auto flex w-full max-w-3xl min-h-0 flex-1 flex-col">
                  ...
                  <div className="min-h-0 flex-1 flex flex-col">
                    <ChatPanel
                      ...
                      feedItems={feedItems}
                      onSend={(text, _files) => void handleSend(text)}
